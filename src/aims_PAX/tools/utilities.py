...
def update_model_auxiliaries(
    ...
    if update_atomic_energies:
        ...
        atomic_energies = torch.tensor(
            atomic_energies, dtype=dtype_mapping[dtype]
        )
...
def save_datasets(
    ensemble: dict, ensemble_ase_sets: dict, path: str, initial: bool = False
):
    """
...
def atoms_full_copy(atoms: ase.Atoms):
...
def save_ensemble(ensemble: dict, training_setups: dict, mace_settings: dict):
    for tag, model in ensemble.items():
        param_context = (
    ...
    def __call__(self, driver, metric, idx=None) -> Any:
        if self.settings["type"] == "temp":
            if metric in self.mod_interval:
                ...
                logging.info(
                    f"Changed temperature by {self.temp_step} to {round(driver.temp / units.kB,1)} K."
                )
                return True
            else:
                return False
    ...
    def __init__(self, use_mpi=True):
        ...
        if self.use_mpi:
            try:
                ...
                raise ImportError(
                    "mpi4py is not installed. Please install it to use MPI features."
                )
