...
SCHEME = {
    ...
    "optional_misc": {
        ...
        "hirshfeld_ratios_key": "REF_hirshfeld_ratios",
...
def check_aimsPAX_settings(settings: dict, procedure: str = "full") -> dict:
    ...
    if procedure == "al" or procedure == "full":
        ...
        # Validate initial dataset paths if provided
        train_dataset = al_settings.get("initial_train_dataset")
        valid_dataset = al_settings.get("initial_valid_dataset")

        if train_dataset is not None or valid_dataset is not None:
            # Validate initial_train_dataset
            if isinstance(train_dataset, (str, Path)):
                train_path = Path(train_dataset)
                assert train_path.exists(), (
                    f"initial_train_dataset path does not exist: "
                    f"{train_path}"
                )
                assert train_path.suffix in [".xyz", ".extxyz"], (
                    f"initial_train_dataset path must be .xyz or "
                    f".extxyz, got: {train_path}"
                )
            elif isinstance(train_dataset, dict):
                for path_str in train_dataset.values():
                    path = Path(path_str)
                    assert path.exists(), (
                        f"initial_train_dataset path does not exist: "
                        f"{path}"
                    )
                    assert path.suffix in [".xyz", ".extxyz"], (
                        f"initial_train_dataset path must be .xyz or "
                        f".extxyz, got: {path}"
                    )

            # Validate initial_valid_dataset
            if isinstance(valid_dataset, (str, Path)):
                valid_path = Path(valid_dataset)
                assert valid_path.exists(), (
                    f"initial_valid_dataset path does not exist: "
                    f"{valid_path}"
                )
                assert valid_path.suffix in [".xyz", ".extxyz"], (
                    f"initial_valid_dataset path must be .xyz or "
                    f".extxyz, got: {valid_path}"
                )
            elif isinstance(valid_dataset, dict):
                for path_str in valid_dataset.values():
                    path = Path(path_str)
                    assert path.exists(), (
                        f"initial_valid_dataset path does not exist: "
                        f"{path}"
    ...
    for idx, md_setting in md_settings.items():
        ...
        if md_setting["stat_ensemble"].lower() == "npt":
            if md_setting["barostat"].lower() not in [
                "berendsen",
                "mtk",
                "isomtk",
            ]:
