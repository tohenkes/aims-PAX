...
) -> tuple[dict[int, AllowedMDMakers], AllowedReferenceMakers]:
    """Returns initialized md and reference makers from project and model settings."""
    available_md_makers = {
        ("foundational", "mace-mp"): get_mace_mp_maker,
    }
    model_used = (idg_settings.initial_sampling, idg_settings.foundational_model)

    if model_used not in available_md_makers:
        raise NotImplementedError(
            f"{model_used} MD model sampling is not currently supported"
            f" in atomate2 aims-PAX."
        )
    md_maker = available_md_makers[model_used](
        {**idg_settings.foundational_model_settings.model_dump(), **model_settings},
        md_settings,
    )
    ref_maker = get_aims_maker(
        misc_settings.path_to_control, idg_settings.species_dir
    )
    return md_maker, ref_maker


def get_mace_mp_maker(
        model_settings: dict[str, Any],
        md_settings: MDSettings,
    ...
    idx_makers = (
        ...
        if isinstance(md_settings.root, dict)
        else [1]
    )
    valid_dynamics = {
    ...
    for idx in idx_makers:
        ...
        if ensemble == "nvt":
            ...
            # AseMDMaker expects pressure in kBar
            kwargs["pressure"] = md_setting.pop("pressure") / 1e8
