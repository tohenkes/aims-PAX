...
@pytest.fixture(scope="module")
def valid_loader_and_output_args(si_model_and_data):
    """Shared valid loader and output_args for test_model tests."""
    _, _, valid_atoms, z_table, r_max, keyspec = si_model_and_data
    loader = make_loader(
...
@pytest.mark.slow
def test_test_model_metrics_finite(
    si_model_and_data, valid_loader_and_output_args
):
    """eval_test_model returns a metrics dict with finite mae_e and mae_f."""
    model, _, _, _, _, _ = si_model_and_data
    loader, output_args = valid_loader_and_output_args
...
@pytest.mark.slow
def test_test_model_return_predictions(
    si_model_and_data, valid_loader_and_output_args
):
    """eval_test_model with return_predictions=True includes predictions."""
    model, _, _, _, _, _ = si_model_and_data
    loader, output_args = valid_loader_and_output_args
    result = eval_test_model(
        model=model,
    ...
    assert isinstance(
        preds["forces"], torch.Tensor
    ), "predictions['forces'] should be a Tensor"
    assert (
        preds["forces"].ndim == 2
    ), f"predictions['forces'] should be 2-D, got ndim={preds['forces'].ndim}"
    assert (
        preds["forces"].numel() > 0
    ), "predictions['forces'] must be non-empty"
...
def test_test_ensemble_structure(si_model_and_data):
    """eval_test_ensemble returns (avg_metrics, per_model) finite values."""
    ...
    for tag in ("m0", "m1"):
        ...
        assert "mae_e" in m, f"Missing 'mae_e' in per_model_metrics['{tag}']"
        assert "mae_f" in m, f"Missing 'mae_f' in per_model_metrics['{tag}']"
        assert math.isfinite(
            m["mae_e"]
